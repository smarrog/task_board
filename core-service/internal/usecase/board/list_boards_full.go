...
type BoardFullItem struct {
	Board   *board.Board
	Columns []*column.Column
	Tasks   []*task.Task
}

type ListBoardsFullUseCase struct {
	boards  board.Repository
	columns column.Repository
	tasks   task.Repository
}

type ListBoardsFullInput struct {
	OwnerId string
}

type ListBoardsFullOutput struct {
	Items []*BoardFullItem
}

func NewListBoardsFullUseCase(boards board.Repository, columns column.Repository, tasks task.Repository) *ListBoardsFullUseCase {
	return &ListBoardsFullUseCase{boards: boards, columns: columns, tasks: tasks}
}

func (uc *ListBoardsFullUseCase) Execute(ctx context.Context, input ListBoardsFullInput) (*ListBoardsFullOutput, error) {
	...
	if len(boardsList) == 0 {
		return &ListBoardsFullOutput{Items: []*BoardFullItem{}}, nil
	}
	...
	items := make([]*BoardFullItem, 0, len(boardsList))
	for _, b := range boardsList {
		bid := b.Id().String()
		...
		items = append(items, &BoardFullItem{
			Board:   b,
			Columns: bcols,
			Tasks:   flatTasks,
		})
	...
	return &ListBoardsFullOutput{Items: items}, nil
}
